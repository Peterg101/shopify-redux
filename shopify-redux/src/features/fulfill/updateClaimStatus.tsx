...
export const UpdateClaimStatus= () => {
  ...
  const [quantity, setQuantity] = useState(updateClaimedOrder.quantity);
  const [viewerOpen, setViewerOpen] = useState(false);
  const dispatch = useDispatch();

  const increment = () =>
    setQuantity((q) => (q < updateClaimedOrder.quantity ? q + 1 : q));
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          <Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
            <Typography variant="body1">
              <strong>Name:</strong> {updateClaimedOrder.name}
            </Typography>
            <Typography variant="body1">
              <strong>Material:</strong> {updateClaimedOrder.material}
            </Typography>
            <Typography variant="body1">
              <strong>Technique:</strong> {updateClaimedOrder.technique}
            </Typography>
            <Typography variant="body1">
              <strong>Colour:</strong> {updateClaimedOrder.colour}
            </Typography>
            <Typography variant="body1">
              <strong>Size:</strong> {updateClaimedOrder.sizing}
            </Typography>
            <Typography variant="body1">
              <strong>Quantity:</strong> {updateClaimedOrder.quantity}
            </Typography>
            {/* <Typography variant="body1">
              <strong>Price (per unit):</strong> ${claimedOrder.price.toFixed(2)}
            </Typography> */}
            <Typography variant="body1">
              <strong>Status:</strong> {updateClaimedOrder.status}{" "}
              {updateClaimedOrder.is_collaborative && "(Collaborative)"}
            </Typography>
          </Box>
        </Paper>
        ...
        >
          ...
          <Typography
            ...
            mb={3}
          >
            You can claim up to <strong>{updateClaimedOrder.quantity}</strong> items from
            this order.
          </Typography>
          ...
          >
            ...
            <TextField
              ...
              onChange={(e) => {
                ...
                if (!isNaN(val) && val >= 1 && val <= updateClaimedOrder.quantity) {
                  setQuantity(val);
                }
              }}
              inputProps={{
                min: 1,
                max: updateClaimedOrder.quantity,
            ...
            <IconButton
              onClick={increment}
              color="primary"
              disabled={quantity >= updateClaimedOrder.quantity}
              sx={{
                bgcolor: "#f1f3f5",
                "&:hover": { bgcolor: "#e9ecef" },
              }}
      ...
      <Dialog open={viewerOpen} onClose={() => setViewerOpen(false)} maxWidth="lg" fullWidth>
        <DialogTitle>{updateClaimedOrder.name}</DialogTitle>
        <Box>
          <OBJSTLViewer />
        </Box>
        <DialogActions>
