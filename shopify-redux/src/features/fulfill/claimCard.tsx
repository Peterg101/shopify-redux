...
export const ClaimCard: React.FC<Order> = (order) => {
   ...
   */
  const updateClaimStatus = async (order: Order) => {
    try {
  ...
  return (
    <Card sx={{ mb: 2 }}>
      <CardContent>
        <Typography variant="h6">CLAIMED ORDER</Typography>
        ...
        <Typography variant="body2" color="text.secondary">
         ...
         <Typography variant="body2" color="text.secondary">
           Total value to you: ${((order.price/order.quantity)*order.quantity_claimed).toFixed(2)}
        </Typography>
        ...
        {(
          <Button
            size="small"
            variant="contained"
            onClick={() => updateClaimStatus(order)}
          >
            Update Claim Status
          </Button>
        )}
