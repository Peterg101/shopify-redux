...
def get_pending_disbursement(
    ...
    # Look up the related order for payment tracking
...
@app.patch("/orders/by_payment_intent/{payment_intent_id}/status")
...
@app.patch("/disbursements/by_payment_intent/{payment_intent_id}/freeze")
def freeze_disbursements_by_payment_intent(
    payment_intent_id: str,
    payload: dict = Depends(verify_jwt_token),
    db: Session = Depends(get_db),
):
    # Find all orders with this payment_intent, then their claims, then pending disbursements
    orders = db.query(Order).filter(Order.payment_intent == payment_intent_id).all()
    if not orders:
        raise HTTPException(status_code=404, detail="No orders found for this payment_intent")
